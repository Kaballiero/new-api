...
func TestGeneratedAdminContractsMatchResolvedHandlers(t *testing.T) {
	...
	assert.Equal(t, map[string]interface{}{"type": "array", "items": map[string]interface{}{"$ref": "#/components/schemas/EffectivePricingTier"}}, groupProps["tiers"])
	...
	assert.Equal(t, map[string]interface{}{"type": "array", "items": map[string]interface{}{"$ref": "#/components/schemas/EffectiveUnitPrice"}}, tierProps["unit_prices"])
	assert.Equal(t, map[string]interface{}{"$ref": "#/components/schemas/EffectivePricingTierCondition"}, tierProps["condition"])
	assert.Equal(t, []string{"unit_prices"}, tier["required"])
	conditionProps := components["schemas"].(map[string]interface{})["EffectivePricingTierCondition"].(map[string]interface{})["properties"].(map[string]interface{})
	for _, field := range []string{"kind", "unit", "min_value", "max_value", "time_zone", "time_windows"} {
		assert.Contains(t, conditionProps, field)
	}
