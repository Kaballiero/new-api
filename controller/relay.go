...
func getChannel(c *gin.Context, info *relaycommon.RelayInfo, retryParam *service.RetryParam) (*model.Channel, *types.NewAPIError) {
	...
	if err != nil {
		return nil, types.NewError(fmt.Errorf("failed to get an available channel for model %[2]s in group %[1]s (retry): %[3]s", selectGroup, info.OriginModelName, err.Error()), types.ErrorCodeGetChannelFailed, types.ErrOptionWithSkipRetry())
	}
	if channel == nil {
		return nil, types.NewError(fmt.Errorf("no available channel for model %[2]s in group %[1]s (retry)", selectGroup, info.OriginModelName), types.ErrorCodeGetChannelFailed, types.ErrOptionWithSkipRetry())
	}
...
func RelayMidjourney(c *gin.Context) {
	...
	if mjErr != nil {
		...
		if mjErr.Code == 30 {
			mjErr.Result = "The current group is at full capacity. Please try again later, or upgrade your account for better service quality."
...
func respondTaskError(c *gin.Context, taskErr *taskdto.TaskError) {
	if taskErr.StatusCode == http.StatusTooManyRequests {
		taskErr.Message = "The upstream for the current group is at full capacity. Please try again later."
	}
